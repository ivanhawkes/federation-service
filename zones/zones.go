...
func (api ZoneApi) Register() {
	...
	ws.Route(ws.POST("").To(api.create).
		...
		Param(ws.BodyParameter("Zone", "representation of a zone").DataType("zones.Zone")).
		Reads(Zone{}))

	ws.Route(ws.GET("/{zone-id}").To(api.read).
		// Swagger documentation.
		Doc("read a zone").
		Param(ws.PathParameter("zone-id", "identifier for a zone").DataType("string")).
		Writes(Zone{}))

	ws.Route(ws.PUT("/{zone-id}").To(api.update).
		// Swagger documentation.
		Doc("update an existing zone").
		Param(ws.PathParameter("zone-id", "identifier for a zone").DataType("string")).
		Param(ws.BodyParameter("Zone", "representation of a zone").DataType("zones.Zone")).
		Reads(Zone{}))

	ws.Route(ws.DELETE("/{zone-id}").To(api.delete).
		// Swagger documentation.
		Doc("delete a zone").
		Param(ws.PathParameter("zone-id", "identifier for a zone").DataType("string")))
...
//
func (api *ZoneApi) create(r *restful.Request, w *restful.Response) {
	...
	// The resource belongs to this zone.
...
// Read the resource.
//
func (api ZoneApi) read(r *restful.Request, w *restful.Response) {
	c := appengine.NewContext(r.Request)

	// Decode the request parameter to determine the key for the entity.
	k, err := datastore.DecodeKey(r.PathParameter("zone-id"))
	if err != nil {
...
// Update the resource.
//
func (api *ZoneApi) update(r *restful.Request, w *restful.Response) {
	c := appengine.NewContext(r.Request)
...
// Delete the resource.
//
func (api *ZoneApi) delete(r *restful.Request, w *restful.Response) {
	c := appengine.NewContext(r.Request)
