...
func init() {
	log.Printf("Missions: Register")
}

// Register the routes we require for this resource type.
//
func (api MissionApi) Register() {
	...
	ws.Route(ws.POST("").To(api.create).
		...
		Param(ws.BodyParameter("Mission", "representation of a faction").DataType("missions.Mission")).
		Reads(Mission{}))
...
//
func (api *MissionApi) create(r *restful.Request, w *restful.Response) {
	...
	// Store the faction.
	k, err := datastore.Put(c, datastore.NewIncompleteKey(c, "missions", ancestor), f)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// Read the resource.
//
func (api MissionApi) read(r *restful.Request, w *restful.Response) {
	c := appengine.NewContext(r.Request)
...
// Update the resource.
//
func (api *MissionApi) update(r *restful.Request, w *restful.Response) {
	c := appengine.NewContext(r.Request)
...
// Delete the resource.
//
func (api *MissionApi) delete(r *restful.Request, w *restful.Response) {
	c := appengine.NewContext(r.Request)
